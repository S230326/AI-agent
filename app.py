...
a = st.number_input(' 어떤 종류의 여정인가요?(개인적인 여행이면 0, 비즈니스 여행이면 1로 입력해주세요)  ')   # 사용자 입력
b = st.number_input(' 어떤 좌석에 탑승하셨나요?(이코노미면 0, 이코노미 플러스면 1, 비즈니스면 2로 입력해주세요) ')
c = st.number_input(' 비행 중 오락 시설이 있나요?(만족도를 1~5까지 입력해주세요) ')
